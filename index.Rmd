...
#Final output table
OUT <- matrix(ncol = 9, nrow = sum(SumDat))
colnames(OUT) <- c("BP_ID","GOTerm", "Term", "TAIR", "Evidence","ENTREZ", "Gene_Name", "Gene_Definition", "Gene_Aliases")
...
# Reduce dataset to only one occurrence per gene per pathway per GO term
redOUT <- OUT[!duplicated(OUT[,c(1,2,4)]),]

#How many genes are shared between the pathways?
...
#Plot GO graph
queryNodes <- unique(OUT$GOTerm)
goGrOUT <- GOstats::GOGraph(queryNodes, GOBPCHILDREN)

nodes <- buildNodeList(goGrOUT)
nAttrs <- list()
nAttrs$fillcolor <- ifelse(nodes(goGrOUT) %in% queryNodes, "red", "white")
names(nAttrs$fillcolor) <- nodes(goGrOUT)
nAttrs$label <- names(nodes(goGrOUT)); names(nAttrs$label) <- names(nodes(goGrOUT))
pg <- plot(goGrOUT, nodeAttrs = nAttrs)
